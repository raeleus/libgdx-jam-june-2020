...
public class MoveStahlfelgeSkull extends MoveSpecialTemplate {
    private static final float PROJECTILE_SPEED = 1000f;
    private static final float PROJECTILE_ACCELERATION = 0f;
    
    public MoveStahlfelgeSkull() {
        airFriction = 0f;
        groundFriction = 2400f;
        attackAnim = STAHLFELGE_SKULL.animation;
        gravity = 2000f;
        hSpeed = 0f;
        vSpeed = 0f;
    }
    ...
    public void update(PerformerEntity performer, float delta) {
        ...
        if (performer.soundEvent) {
            Sound sound = assetManager.get("sfx/fireball.mp3");
            sound.play();
        }
    }
}
